...
export async function POST(request: Request) {
  ...
  if (!checkRateLimit(`pdf-to-powerpoint:${userId}`)) {
  ...
  if (uploadedFile.type !== "application/pdf" && !lowerName.endsWith(".pdf")) {
    return jsonError("PDF to PowerPoint accepts PDF uploads only.", 415);
  }
  if (uploadedFile.size > MAX_UPLOAD_BYTES) {
    return jsonError("PDF to PowerPoint uploads are limited to 100 MB.", 413);
  }

  const tempDir = await mkdtemp(join(tmpdir(), "wiserfiles-pdf2pptx-"));
  const sanitized = normalizeUploadName(uploadedFile.name);
  const inputPath = join(tempDir, `${sanitized}.pdf`);
  const outputPath = join(tempDir, `${sanitized}.pptx`);
  const script = join(process.cwd(), "scripts", "pdf2powerpoint.py");
  ...
  try {
    ...
    return new Response(outputBytes, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${sanitized}.pptx"`,
        "Cache-Control": "no-store",
        "X-Conversion-Engine": "pymupdf-typography",
      },
    });
  } catch (error) {
    return jsonError(`PDF to PowerPoint conversion failed: ${stderrOf(error).split("\n").slice(-1)[0] || "unknown failure"}`, 500);
  } finally {
