...
export async function POST(request: Request) {
  ...
  if (!checkRateLimit(`pdf-to-pdfa:${userId}`)) {
  ...
  if (uploadedFile.type !== "application/pdf" && !lowerName.endsWith(".pdf")) {
    return jsonError("PDF to PDF/A accepts PDF uploads only.", 415);
  }
  if (uploadedFile.size > MAX_UPLOAD_BYTES) {
    return jsonError("PDF to PDF/A uploads are limited to 100 MB.", 413);
  }

  const tempDir = await mkdtemp(join(tmpdir(), "wiserfiles-pdfa-"));
  const sanitized = normalizeUploadName(uploadedFile.name);
  const inputPath = join(tempDir, `${sanitized}.pdf`);
  const outputPath = join(tempDir, `${sanitized}-pdfa.pdf`);

  try {
    await writeFile(inputPath, Buffer.from(await uploadedFile.arrayBuffer()));

    // Ghostscript PDF/A-2b conversion (archival, self-contained PDF/A).
    await execFileAsync(
      "gs",
      [
        "-dPDFA=2",
    ...
    return new Response(outputBytes, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${sanitized}-pdfa.pdf"`,
        "Cache-Control": "no-store",
        "X-Conversion-Engine": "ghostscript-pdfa-2b",
      },
    });
  } catch (error) {
    return jsonError(`PDF to PDF/A conversion failed: ${stderrOf(error).split("\n").slice(-1)[0] || "unknown failure"}`, 500);
  } finally {
