...
# Run as a non-root user. No passwordless sudo is granted: user-submitted
# code runs in this container, and sudo + a package manager is a documented
# privilege-escalation path. All TeX collections are pre-installed above, and
# missing .sty packages install into the app user's own texmf tree via tlmgr
# (no root required).
RUN apt-get update \
 ...
 && pip3 install --no-cache-dir --break-system-packages pdf2docx==0.5.13 pymupdf==1.28.2 python-docx==1.2.0 openpyxl==3.1.5 python-pptx==1.0.2 pytest==8.3.4 \
...
# then drop privileges. Missing .sty packages install into ~app/texmf via tlmgr
# (user tree, no root).
