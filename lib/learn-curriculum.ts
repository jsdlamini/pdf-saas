...
export const LEARN_LESSONS: LearnLesson[] = [
  ...
  },
  {
    ...
    title: "Dictionaries",
    explanation:
      "A **dictionary** maps keys to values — like a real dictionary maps words to meanings.\n\n- `{}` creates one: `d = {'maize': 12}`\n- Read a value: `d['maize']`\n- Add or change: `d['beans'] = 15`\n- Check membership: `'maize' in d`\n- Loop: `for k, v in d.items():`\n\nUse `d.get(key, default)` to read safely when a key might be missing.",
    example: 'prices = {"maize": 12, "beans": 15, "rice": 20}\nprint(prices["maize"])\nprices["sorghum"] = 14\nfor item, price in prices.items():\n    print(item, price)\n',
    challenges: [
      py("Create `d = {'a': 1, 'b': 2}` and print `d['b']`.", "2"),
      py("Create `prices = {'maize': 12}` and print `prices['maize']`.", "12"),
      py("Create an empty dict `d = {}`, set `d['x'] = 5`, then print `d['x']`.", "5"),
      py("Create `d = {'n': 1}`, change `d['n'] = 9`, and print it.", "9"),
      py("Print `len({'a': 1, 'b': 2, 'c': 3})`.", "3"),
      py("Print `'a' in {'a': 1, 'b': 2}` (a True/False).", "True"),
      py("Print `{'a': 1}.get('missing', 0)`.", "0"),
      py("Print `sum({'a': 10, 'b': 20}.values())`.", "30"),
      py("Print each key of `{'a': 1, 'b': 2}` using a loop, one per line.", "a\nb"),
      py("Print `max({'a': 3, 'b': 9, 'c': 5}.values())`.", "9"),
      wordPy("A store's price list maps items to prices: maize is 12, beans is 15, rice is 20. Build a dictionary for it and print the price of beans.", "15"),
      wordPy("A school records the number of students per class in a dictionary: Grade1 is 30, Grade2 is 28, Grade3 is 32. Print the total number of students.", "90"),
      wordPy("A poll counts votes for three candidates: A got 120, B got 95, C got 60. Store the votes in a dictionary and print the name of the winning candidate.", "A"),
    ],
  },
  {
    id: "py-functions",
  ...
  },
  {
    ...
    title: "Classes",
    explanation:
      "A **class** is a blueprint for objects.\n\n- `class Name:` defines one\n- `__init__(self, ...)` is the constructor that stores attributes\n- methods are functions that take `self` first\n\nCreate an instance with `Name(...)` and access members with a dot: `obj.attr`, `obj.method()`.",
    example: 'class Farmer:\n    def __init__(self, name, plots):\n        self.name = name\n        self.plots = plots\n    def describe(self):\n        return f"{self.name} has {self.plots} plots"\n\nf = Farmer("Sam", 3)\nprint(f.describe())\n',
    challenges: [
      py("Define `class Point` with `__init__(self, x)` storing `self.x`; make `p = Point(7)` and print `p.x`.", "7"),
      py("Define `class Box` with `__init__(self, w, h)`; make `b = Box(3, 4)` and print `b.w`.", "3"),
      py("Define `class Circle` with `__init__(self, r)`; make `c = Circle(5)` and print `c.r`.", "5"),
      py("Define `class Person` with `__init__(self, name)`; make `p = Person('Sam')` and print `p.name`.", "Sam"),
      py("Define `class Counter` with `__init__(self)` setting `self.n = 0`; make `c = Counter()` and print `c.n`.", "0"),
      py("Define `class Doubler` with a method `double(self, x)` returning `x * 2`; print `Doubler().double(21)`.", "42"),
      py("Define `class Adder` with a method `add(self, a, b)` returning `a + b`; print `Adder().add(10, 32)`.", "42"),
      py("Define `class Rect` with `area(self)` returning `self.w * self.h`; print `Rect(8, 5)` after setting w=8, h=5.", "40"),
      py("Define `class Greeter` with `hi(self)` returning 'Hello'; print `Greeter().hi()`.", "Hello"),
      py("Define `class Square` with `area(self)` returning `self.side * self.side`; print `Square(9)`'s area after setting side=9.", "81"),
      wordPy("A bank account starts with a balance and can be deposited into. Define a class `Account` with `__init__(self, balance)` and a method `deposit(self, amount)` that adds to the balance. Start at 100, deposit 250, and print the balance.", "350"),
      wordPy("A rectangle has a width and height. Define a class `Rect` with a method `area(self)` returning width times height. Create a rectangle of width 6 and height 7 and print its area.", "42"),
      wordPy("A student has a name and a list of three marks. Define a class `Student` with a method `average(self)` returning the mean of the marks. Create one with marks 70, 80 and 90, and print the average as a whole number.", "80"),
    ],
  },
  {
    id: "py-fizzbuzz",
  ...
  },
  {
    ...
    title: "Maps",
    explanation:
      "A `std::map` is C++'s key-to-value store (like a dictionary).\n\n- `#include <map>`\n- `std::map<std::string, int> m;`\n- Write: `m[\"maize\"] = 12;`\n- Read: `m[\"maize\"]`\n- Loop: `for (auto& p : m)` — `p.first` is the key, `p.second` the value.",
    example: '#include <iostream>\n#include <map>\n#include <string>\nint main() {\n    std::map<std::string, int> prices;\n    prices["maize"] = 12;\n    prices["beans"] = 15;\n    std::cout << prices["maize"] << std::endl;\n    for (auto& p : prices) std::cout << p.first << " " << p.second << std::endl;\n    return 0;\n}\n',
    challenges: [
      cpp("Create `std::map<std::string,int> m;`, set `m[\"a\"] = 1` and `m[\"b\"] = 2`, and print `m[\"b\"]`.", "2"),
      cpp("Create a map with `m[\"maize\"] = 12;` and print `m[\"maize\"]`.", "12"),
      cpp("Create an empty map, set `m[\"x\"] = 5;`, then print `m[\"x\"]`.", "5"),
      cpp("Create `m[\"n\"] = 1;`, then `m[\"n\"] = 9;`, and print `m[\"n\"]`.", "9"),
      cpp("Create `m[\"a\"] = 1; m[\"b\"] = 2; m[\"c\"] = 3;` and print `m.size()`.", "3"),
      cpp("Print the sum of the values in a map with `m[\"a\"] = 10; m[\"b\"] = 20;`.", "30"),
      cpp("Create `m[\"a\"] = 1; m[\"b\"] = 2;` and print each key on its own line using a loop.", "a\nb"),
      cpp("Create `m[\"a\"] = 3; m[\"b\"] = 9; m[\"c\"] = 5;` and print the largest value.", "9"),
      cpp("Create `m[\"maize\"] = 12; m[\"beans\"] = 15;` and print the sum of the values.", "27"),
      cpp("Create a map with `m[\"x\"] = 42;` and print `m.count(\"x\")` (1 if present).", "1"),
      wordCpp("A store's price list maps items to prices: maize is 12, beans is 15, rice is 20. Build a `std::map` for it and print the price of beans.", "15"),
      wordCpp("A school records students per class in a map: Grade1 is 30, Grade2 is 28, Grade3 is 32. Print the total number of students.", "90"),
      wordCpp("A poll counts votes: A got 120, B got 95, C got 60. Store the votes in a map and print the name of the candidate with the most votes.", "A"),
    ],
  },
  {
    id: "cpp-functions",
  ...
  },
  {
    ...
    title: "Classes",
    explanation:
      "A **class** bundles data and functions into one type.\n\n```cpp\nclass Box {\npublic:\n    int w, h;\n    Box(int a, int b) : w(a), h(b) {}\n    int area() { return w * h; }\n};\n```\n\nCreate an instance with `Box b(3, 4);` and call members with a dot.",
    example: '#include <iostream>\n#include <string>\nclass Farmer {\npublic:\n    std::string name;\n    int plots;\n    Farmer(std::string n, int p) : name(n), plots(p) {}\n    std::string describe() { return name + " has " + std::to_string(plots) + " plots"; }\n};\nint main() {\n    Farmer f("Sam", 3);\n    std::cout << f.describe() << std::endl;\n    return 0;\n}\n',
    challenges: [
      cpp("Define `class Point { public: int x; Point(int a) : x(a) {} };`, make `Point p(7);`, print `p.x`.", "7"),
      cpp("Define `class Box { public: int w, h; Box(int a, int b) : w(a), h(b) {} };`, make `Box b(3, 4);`, print `b.w`.", "3"),
      cpp("Define `class Circle { public: int r; Circle(int a) : r(a) {} };`, make `Circle c(5);`, print `c.r`.", "5"),
      cpp("Define `class Counter { public: int n; Counter() : n(0) {} };`, make `Counter c;`, print `c.n`.", "0"),
      cpp("Define a class with a method `int twice(int x) { return x * 2; }`; print `C().twice(21)`.", "42"),
      cpp("Define a class with a method `int add(int a, int b) { return a + b; }`; print `C().add(10, 32)`.", "42"),
      cpp("Define `class Rect { public: int w, h; Rect(int a, int b) : w(a), h(b) {} int area() { return w * h; } };`, print `Rect(8, 5).area()`.", "40"),
      cpp("Define `class Square { public: int side; Square(int s) : side(s) {} int area() { return side * side; } };`, print `Square(9).area()`.", "81"),
      cpp("Define a class with a method returning the string `Hello`; print it.", "Hello"),
      cpp("Define `class Account { public: int bal; Account(int b) : bal(b) {} void deposit(int a) { bal += a; } };`, start at 100, deposit 250, print `bal`.", "350"),
      wordCpp("A bank account starts with a balance and can be deposited into. Define `class Account` with a constructor and a `deposit(int amount)` method. Start at 100, deposit 250, and print the balance.", "350"),
      wordCpp("A rectangle has a width and height. Define `class Rect` with an `area()` method returning width times height. Create one of width 6 and height 7 and print its area.", "42"),
      wordCpp("A student has three marks. Define `class Student` with a method that returns the whole-number average. Use marks 70, 80 and 90, and print the average.", "80"),
    ],
  },
  {
    id: "cpp-fizzbuzz",
